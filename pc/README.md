...
11 字节，v3 遥测帧共 79 字节；接收器可处理分包、粘包、错位和 CRC 错包。
当前协议版本为 3；IMU 角速度以 `0.1°/s`、角度以 `0.01°` 传输，界面会
自动恢复成带小数的物理量显示。

点击主界面的“实时曲线 / 参数调节”，可以查看灰度位置、IMU 航向闭环和
左右编码器目标/实测速度，并通过 ESP-NOW 在线修改控制参数。窗口还可以把
最近约一分钟的完整遥测导出为 CSV，供后续 Python 离线分析。
